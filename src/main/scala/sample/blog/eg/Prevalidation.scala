...
//runMain sample.blog.eg.Betting
object Prevalidation extends App {
    ...
    Future {
      ...
      ordCmd.origin match {
        case Some(cmd) ⇒
          ...
          LockSupport.parkNanos(ThreadLocalRandom.current.nextLong(50L, 100L))
  ...
  def preValidation0(
    ...
    //akka.stream.contrib.PassThroughFlow(???)
    ???
  ...
  def preValidation(
    ...
    Flow[Cmd]
      .scan(OrderedCmd(0L)) { (ordCmd, cmd) ⇒ ordCmd.withOrigin(cmd) }
      .mapAsyncUnordered(parallelism) { cmd ⇒ preValidateCmd(cmd)(mat.executionContext) }
  ...
  def preValidationFlow(bufferSize: Int = 1 << 8) = {
    /*Source
      .fromFuture(Future{ 1 })
      .flatMapConcat {
        Source.queue[Cmd](bufferSize, OverflowStrategy.dropHead)
      }*/
